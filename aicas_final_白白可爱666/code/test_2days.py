import sys
sys.path.append('/tianchi_docker')
...
def read_user_table(file):
    # 读取用户行为数据
    user_table = pd.read_csv(file, sep="\t",usecols=[0,1,2,4,5],header=None,names=["user_id","item_id","behavior_type","item_category","time"],dtype={'behavior_type':np.int8,"item_category":np.int16},nrows=1000000)
...
# file_list = ['../tcdata/round2_user_0.txt', '../tcdata/round2_user_1.txt', '../tcdata/round2_user_2.txt', '../tcdata/round2_user_3.txt', '../tcdata/round2_user_4.txt']
...
def test_2days(user_table,FEATURE_EXTRACTION_SLOT):
    print("开始读取2DAYS")
    def get_label_testset(train_user,LabelDay,return_dict):
        # 测试集选为上一天所有的交互数据
        data_test = train_user[(train_user['daystime'] == (LabelDay-datetime.timedelta(days=1)))]#&((train_user.behavior_type==3)|(train_user.behavior_type==2))
    ...
    def item_category_feture(data,end_time,beforeoneday,return_dict):
        ...
        # item_category_feture = pd.merge(item_category_feture,buyRate_2,how='left',right_index=True,left_index=True,suffixes=('_left322', '_right322'))
        # item_category_feture = pd.merge(item_category_feture,buyRate_3,how='left',right_index=True,left_index=True,suffixes=('_left332', '_right331'))
        # item_category_feture = pd.merge(item_category_feture,buyRate_5,how='left',right_index=True,left_index=True,suffixes=('_left312', '_right321'))
    ...
    def item_id_feture(data,end_time,beforeoneday,return_dict):   
        ...
        # item_id_feture = pd.merge(item_id_feture,buyRate_2,how='left',right_index=True,left_index=True,suffixes=('_left322', '_right322'))
        # item_id_feture = pd.merge(item_id_feture,buyRate_3,how='left',right_index=True,left_index=True,suffixes=('_left332', '_right331'))
        # item_id_feture = pd.merge(item_id_feture,buyRate_5,how='left',right_index=True,left_index=True,suffixes=('_left312', '_right321'))
    ...
    def user_id_feture(data,end_time,beforeoneday,return_dict):   
        ...
        # user_id_feture = pd.merge(user_id_feture,buyRate_2,how='left',right_index=True,left_index=True,suffixes=('_left322', '_right322'))
        # user_id_feture = pd.merge(user_id_feture,buyRate_3,how='left',right_index=True,left_index=True,suffixes=('_left332', '_right331'))
        # user_id_feture = pd.merge(user_id_feture,buyRate_5,how='left',right_index=True,left_index=True,suffixes=('_left312', '_right321'))
    ...
    def user_liveday(train_user_window1,return_dict):#用户各个行为活跃的天数
        ...
        user_live = user_live.droplevel(level=0, axis=1)
    ...
    def build_test(Data):
        ...
        p1.start()
        train_user_window1 =  Data[(Data['daystime'] > (LabelDay - datetime.timedelta(days=FEATURE_EXTRACTION_SLOT-1))) & (Data['daystime'] <= LabelDay - datetime.timedelta(days=1))]
        beforeoneday = Data[Data['daystime'] == (LabelDay-datetime.timedelta(days=1))]
    ...
    print("------start test------")
    bst = xgb.Booster()
    bst.load_model('../weight/model_4depth_2days.bin')
    ...
    predict2 = predicted.iloc[:50000, [0, 1,2]]
    # 保存到文件
    predict2.to_csv(("./result_2days" + ".txt"), header=None, index=False,sep='\t')
    return predict2